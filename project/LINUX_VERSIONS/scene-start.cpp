...
// QuestionJ(1) DELECT OBJECT FUNCTION 
static void deleteObject(int id) {
    sceneObjs[currObject].meshId = NULL;
    currObject = -1;
}

// QuestionJ(2) DUPLICAT OBJECT FUNCTION
...
void drawMesh(SceneObject sceneObj)
{
    ...
    // QuestionB: WHERE .angles and .loc ARE STORED IN THE DATA STRUCTURE
    mat4 rotationB = RotateX(-sceneObj.angles[0])*RotateY(sceneObj.angles[1]) * RotateZ(sceneObj.angles[2]);
    // ADD XYZ AXES ROTATE TO model
...
void display( void )
{
    ...
    /*QuestionA : move it forwards and backwards using the scroll wheel, via the viewDist variable
    ...
    //QuestionI: ADDING THE ROTATE TO THE SECOND LIGHT
    ...
    //QuestionH. PASS BOTH Brightness and Color OF LIGHTS
    ...
    for (int i=0; i < nObjects; i++) {
        ...
        vec3 rgb = so.rgb * so.brightness * 4.0; // INCREASE THE BRIGHTNESS
...
//----------------------------------------------------------------------------
// QuestionC
// FUNCTION FOR ADJUSTING AMBIENT AND DIFFUSE 
...
// FUNCTION FOR ADJUSTING SPECULAR AND SHINE 
...
static void materialMenu(int id)
{
    ...
    // QuestionC: when id == Ambient/Diffuse/Specular/Shine
    else if (id == 20)
    {
        toolObj = currObject;
...
static void mainmenu(int id)
{
    ...
    else if (id == 50){doRotate();}
    //QuestionJ(1) : THE SCENE CAN HAVE NO OBJECT
    else if (id == 00 && currObject > 0) {deleteObject(currObject);} 
    //QuestionJ(2) : CALL duplicateObject()
...
static void makeMenu()
{
    ...
    // QuestionC: changed to implemented
    ...
    //QuestionJ(1): ADDING THE "DELETE OBJECT" TO THE MENU
    glutAddMenuEntry("Delete object", 00);
    //QuestionJ(2): ADDING THE "DUPLICATE OBJECT" TO THE MENU
...
void reshape( int width, int height )
{
    ...
    //QuestionD: nearDist ADJUSTED TO 0.005 SO THE CAMERA CAN GIVE MORE "CLOSE UP" VIEWS OF OBJECTS.
    GLfloat nearDist = 0.005;


    //QuestionE: BEHAVES DIFFERENTLY WHEN THE WIDTH IS LESS THAN THE HEIGHT
    //           REFER TO CITS3003 LAB5 (Q3)
    //           https://teaching.csse.uwa.edu.au/units/CITS3003/labsheet.php?fname=lab5
    if(width<height){       
